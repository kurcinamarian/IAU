...
def get_pipeline_6(df, k):
    ...
    observation_pipeline = Pipeline([
        ...
        #("select_kbest", SelectKBest(score_func=f_regression, k=k).set_output(transform="pandas")), 
        ("RFE", RFE(SVR(kernel="linear"), n_features_to_select=k, step=1).set_output(transform="pandas")),
        ("stack", stack)
    ])
